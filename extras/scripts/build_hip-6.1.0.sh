...
wget -q https://github.com/ROCm/ROCR-Runtime/archive/refs/tags/rocm-6.1.0.tar.gz -O ${ROCM_BRANCH}-rocr-runtime.tgz
wget -q https://github.com/ROCm/ROCT-Thunk-Interface/archive/refs/tags/${ROCM_BRANCH}.tar.gz -O ${ROCM_BRANCH}-roct-thunk-interface.tgz
git clone --recursive https://github.com/ROCm/rocprofiler-register.git rocprofiler-register-${ROCM_BRANCH}
...
tar -xzf ${ROCM_BRANCH}-roct-thunk-interface.tgz
...
export ROCT_THUNK_INTERFACE_DIR="$(readlink -f ROCT-Thunk-Interface-$ROCM_BRANCH)"
export ROCPROFILER_REGISTER_DIR="$(readlink -f rocprofiler-register-$ROCM_BRANCH)"
...
cd "$HIPBLAS_DIR"
sed -i '/git\/hooks/d' install.sh
./install.sh --cmake-arg -DHIP_PLATFORM=nvidia \
             --cmake-arg -DCMAKE_INSTALL_PREFIX=$PREFIX \
...
echo "****************** ROCT_THUNK_INTERFACE **************************"

cd ${ROCT_THUNK_INTERFACE_DIR}
mkdir build && cd build
cmake -DCMAKE_INSTALL_PREFIX=$PREFIX ..
make -j 8
make install

echo "****************** ROCPROFILER_REGISTER_DIR **************************"

cd ${ROCPROFILER_REGISTER_DIR}
mkdir build && cd build
cmake -DCMAKE_INSTALL_PREFIX=$PREFIX ..
make all -j 8
make install
