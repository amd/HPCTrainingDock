...
if [ "${BUILD_FLANGNEW}" = "0" ]; then
	...
	setenv("OMPI_CC","/opt/rocmplus-${ROCM_VERSION}/${ARCHIVE_DIR}/bin/amdclang")
	setenv("OMPI_CXX","/opt/rocmplus-${ROCM_VERSION}/${ARCHIVE_DIR}/bin/amdclang++")
	setenv("OMPI_FC","/opt/rocmplus-${ROCM_VERSION}/${ARCHIVE_DIR}/bin/amdflang-new")
