...
_fftw_on_exit() {
   ...
   # for an operator-writable tree.
   _fail_marker="$(dirname "${FFTW_PATH}")/fftw.FAILED"
   if [ ${rc} -ne 0 ]; then
      ${SUDO} mkdir -p "$(dirname "${FFTW_PATH}")" 2>/dev/null || true
      ${SUDO} tee "${_fail_marker}" >/dev/null 2>/dev/null <<MARKER_EOF || true
FAILED package: fftw
ROCm SDK:        ${ROCM_PATH:-unknown}
ROCm token:      ${ROCM_VERSION:-unknown}
Date:            $(date -u +%Y-%m-%dT%H:%M:%SZ)
Setup script:    fftw_setup.sh (EXIT-trap fail marker)
Reason:          build exited rc=${rc}; partial install wiped (see log_fftw_*.txt).
MARKER_EOF
   else
