...
: ${BUILD_EMACS:="1"}
# turbovnc: ROCm-agnostic remote-desktop server (extras/scripts/turbovnc_setup.sh).
...
: ${BUILD_TURBOVNC:="1"}
# google-chrome: ROCm-agnostic browser module (extras/scripts/google-chrome_setup.sh)
# for viewing Perfetto traces (ui.perfetto.dev) in the no-VNC desktop. Shared across
# ROCm versions (installs under TOP_INSTALL_PATH like emacs/turbovnc, existence-skips
# on later sweep versions). Extracts Google's .deb onto NFS + writes a modulefile; the
# leaf no-ops (SKIPPED) on non-Debian hosts or when dl.google.com is unreachable.
: ${BUILD_GOOGLE_CHROME:="1"}
# qcachegrind: ROCm-agnostic callgrind/cachegrind GUI viewer module
# (extras/scripts/qcachegrind_setup.sh). Built from the KDE source (Qt5) and shipped
# as a self-contained NFS bundle (vendors its own Qt5 + graphviz, like the CubeGUI
# viewer) so no node reimage is needed. Shared across ROCm versions (installs under
# TOP_INSTALL_PATH like emacs/turbovnc, existence-skips on later sweep versions). The
# leaf no-ops (SKIPPED) on non-Debian hosts where the apt Qt5 toolchain is unavailable.
: ${BUILD_QCACHEGRIND:="1"}
...
# umpire: HIP memory-management library, ROCm-version-dependent, installs
# under the per-ROCm rocmplus tree (rocmplus-<suffix>/umpire-v${UMPIRE_VERSION}).
# Migrated leaf: --install-path is the PARENT dir; the leaf appends
# umpire-v${UMPIRE_VERSION}. --module-path carries the pkg subdir (the leaf's
# MODULE_PATH default is .../ROCmPlus/umpire), so pass rocmplus-<suffix>/umpire
# exactly like kokkos above.
run_and_log_versioned umpire extras/scripts/umpire_setup.sh ${COMMON_OPTIONS} --build-umpire ${BUILD_UMPIRE} ${REPLACE_OPTS} \
   $(rocmplus_args rocmplus-${ROCMPLUS_SUFFIX}/umpire)
...
# turbovnc: ROCm-agnostic remote-desktop server. Like emacs/miniconda3 it installs
# under TOP_INSTALL_PATH (shared across ROCm versions) and is intentionally NOT given
# ${REPLACE_OPTS} -- the leaf's existence guard skips the rebuild on every subsequent
# sweep version, and a shared tool should not be wiped per ROCm version. The leaf does
# NOT accept --rocm-version/--amdgpu-gfxmodel, so COMMON_OPTIONS is deliberately not
# threaded. --install-path is a PARENT dir; the leaf appends turbovnc-v${TURBOVNC_VERSION}.
run_and_log_versioned turbovnc extras/scripts/turbovnc_setup.sh --build-turbovnc ${BUILD_TURBOVNC} \
   $([ "${USE_CUSTOM_PATHS}" == 1 ] && echo "--install-path ${TOP_INSTALL_PATH} --module-path ${TOP_MODULE_PATH}/base/turbovnc")

# google-chrome: browser for viewing Perfetto traces in the no-VNC desktop.
# ROCm-agnostic base tool -- installs under TOP_INSTALL_PATH (shared across ROCm
# versions, existence-skips on later sweep versions like emacs/turbovnc) and is
# intentionally NOT given ${REPLACE_OPTS} (a shared tool should not be wiped per
# ROCm version; pass --replace 1 to the leaf, or bump the version, to update
# Chrome). The leaf does NOT accept --rocm-version/--amdgpu-gfxmodel so
# COMMON_OPTIONS is not threaded. --install-path is a PARENT dir; the leaf
# appends google-chrome-v${GOOGLE_CHROME_VERSION}. The leaf no-ops (SKIPPED) on
# non-Debian hosts or when dl.google.com is unreachable, so it never reds a sweep.
run_and_log_versioned google-chrome extras/scripts/google-chrome_setup.sh --build-google-chrome ${BUILD_GOOGLE_CHROME} \
   $([ "${USE_CUSTOM_PATHS}" == 1 ] && echo "--install-path ${TOP_INSTALL_PATH} --module-path ${TOP_MODULE_PATH}/base/google-chrome")

# qcachegrind: Qt-only callgrind/cachegrind GUI viewer -- ROCm-agnostic base tool,
# installs under TOP_INSTALL_PATH (shared across ROCm versions, existence-skips on
# later sweep versions like emacs/turbovnc) and is intentionally NOT given
# ${REPLACE_OPTS} (a shared tool should not be wiped per ROCm version; pass --replace 1
# to the leaf, or bump the version, to update it). The leaf does NOT accept
# --rocm-version/--amdgpu-gfxmodel so COMMON_OPTIONS is not threaded. --install-path is
# a PARENT dir; the leaf appends qcachegrind-v${QCACHEGRIND_VERSION}. It is built from
# the KDE source (Qt5) and vendors its own Qt5 + graphviz onto NFS (self-contained, no
# reimage); the leaf no-ops (SKIPPED) on non-Debian hosts, so it never reds a sweep.
run_and_log_versioned qcachegrind extras/scripts/qcachegrind_setup.sh --build-qcachegrind ${BUILD_QCACHEGRIND} \
   $([ "${USE_CUSTOM_PATHS}" == 1 ] && echo "--install-path ${TOP_INSTALL_PATH} --module-path ${TOP_MODULE_PATH}/base/qcachegrind")
