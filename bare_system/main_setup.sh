...
# (extras/scripts/roofline_extractor_setup.sh). Clones the now-public
# AMD-HPC/rooflineExtractor repo, installs under TOP_INSTALL_PATH (shared
# across ROCm versions, existence-skips on later sweep versions like
# emacs/uprof/miniconda3). The leaf does NOT accept --rocm-version/
# --amdgpu-gfxmodel, so COMMON_OPTIONS is not threaded at its call site.
: ${BUILD_ROOFLINE:="1"}
...
declare -A PKG_FLAG=(
   ...
   [hypre]=BUILD_HYPRE
   [emacs]=BUILD_EMACS
   [roofline-extractor]=BUILD_ROOFLINE
...
# roofline-extractor: AMD Roofline Extractor -- ROCm-agnostic base tool.
# Clones the now-public AMD-HPC/rooflineExtractor repo (anonymous HTTPS),
# vendors its pinned Python deps, and writes a base/roofline-extractor module.
# Like emacs/uprof it installs under TOP_INSTALL_PATH (shared across ROCm
# versions, existence-skips on later sweep versions), so it is NOT given
# ${REPLACE_OPTS} (a shared tool should not be wiped per ROCm version) and the
# leaf does NOT accept --rocm-version/--amdgpu-gfxmodel so COMMON_OPTIONS is
# deliberately not threaded. --install-path is a PARENT dir; the leaf appends
# rooflineExtractor. BUILD_ROOFLINE=0 opt-out + existence check live in
# roofline_extractor_setup.sh.
run_and_log roofline-extractor extras/scripts/roofline_extractor_setup.sh --build-roofline ${BUILD_ROOFLINE} \
