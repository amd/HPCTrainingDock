...
# modulefile are labelled by the per-source default version below (3.2.0
# for the legacy rocm-7.1.x/7.2.x tags, 3.4.0 for the monorepo). These are
# just labels for the dir/module name; override with --rocshmem-version.
...
ROCSHMEM_LEGACY_DEFAULT_VERSION=3.2.0
...
usage()
{
    ...
    echo "  --rocshmem-version [ ROCSHMEM_VERSION ] label for the install dir + modulefile; default per source: ${ROCSHMEM_MONOREPO_DEFAULT_VERSION} (monorepo) or ${ROCSHMEM_LEGACY_DEFAULT_VERSION} (legacy 7.1.x/7.2.x)"
...
# --github-branch override. ROCSHMEM_DEFAULT_VERSION is the label used for
# the install dir + modulefile name when --rocshmem-version is not given.
case "${ROCM_VERSION}" in
   7.1.*|7.2.*)
      ROCSHMEM_SOURCE=legacy
      ROCSHMEM_REPO_URL="${ROCSHMEM_LEGACY_URL}"
      GITHUB_BRANCH="${GITHUB_BRANCH_INPUT:-rocm-${ROCM_VERSION}}"
      ROCSHMEM_DEFAULT_VERSION="${ROCSHMEM_LEGACY_DEFAULT_VERSION}"
      ;;
   *)
      ...
      ROCSHMEM_DEFAULT_VERSION="${ROCSHMEM_MONOREPO_DEFAULT_VERSION}"
      ;;
esac

# ── Resolve the version label ────────────────────────────────────────
# The explicit --rocshmem-version override wins; otherwise use the
# per-source default set above. This is only a label for the dir/module
# name -- the actual sources come from the git ref selected above.
ROCSHMEM_VERSION="${ROCSHMEM_VERSION_INPUT:-${ROCSHMEM_DEFAULT_VERSION}}"
echo "rocshmem: version label ${ROCSHMEM_VERSION} (source: ${ROCSHMEM_SOURCE} @ ${GITHUB_BRANCH})"
