...
# for direct standalone invocation where ROCM_PATH is unset. mdb is
# primarily useful on AMD systems with the rocgdb backend, so we
# preflight rocm/<ver> here -- mdb still works against system gdb if
# the rocm module is missing, but pinning to a specific ROCm SDK in
# the rocmplus tree is the whole point of installing per-rocm-version.
if [[ -n "${ROCM_PATH:-}" ]]; then
   _rp_bn="${ROCM_PATH##*/}"
